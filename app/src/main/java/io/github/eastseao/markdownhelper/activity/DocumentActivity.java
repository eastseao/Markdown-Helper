...
public class DocumentActivity extends MarkdownHelperBaseActivity {

    private Toolbar _toolbar;
    private FragmentManager _fragManager;

    public static void launch(final Activity activity, final Intent intent) {
        final File file = MarkdownHelperContextUtils.getIntentFile(intent);
    ...
    public static void askUserIfWantsToOpenFileInThisApp(final Activity activity, final File file) {
        if (!FormatRegistry.isExternalFile(file) && GsFileUtils.isContentsPlainText(file)) {
            new AlertDialog.Builder(activity, R.style.Theme_AppCompat_DayNight_Dialog_Rounded)
                    ...
                    .setNegativeButton(R.string.other, (dialog1, which) -> new MarkdownHelperContextUtils(activity).viewFileInOtherApp(activity, file, null))
                    .create()
                    .show();
        } else {
            new MarkdownHelperContextUtils(activity).viewFileInOtherApp(activity, file, null);
        }
    }
    ...
    private void handleLaunchingIntent(final Intent intent) {
        ...
        // -----------------------------------------------------------------------
        final File file = MarkdownHelperContextUtils.getIntentFile(intent, this);
