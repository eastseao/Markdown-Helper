...
import io.github.eastseao.markdownhelper.ApplicationObject;
import io.github.eastseao.markdownhelper.BuildConfig;
import io.github.eastseao.markdownhelper.R;
import io.github.eastseao.markdownhelper.format.ActionButtonBase;
import io.github.eastseao.markdownhelper.format.FormatRegistry;
import io.github.eastseao.markdownhelper.format.TextConverterBase;
import io.github.eastseao.markdownhelper.frontend.DraggableScrollbarScrollView;
import io.github.eastseao.markdownhelper.frontend.FileInfoDialog;
import io.github.eastseao.markdownhelper.frontend.MarkdownHelperDialogFactory;
import io.github.eastseao.markdownhelper.frontend.filebrowser.MarkdownHelperFileBrowserFactory;
import io.github.eastseao.markdownhelper.frontend.textview.HighlightingEditor;
import io.github.eastseao.markdownhelper.frontend.textview.LineNumbersView;
import io.github.eastseao.markdownhelper.frontend.textview.TextViewUtils;
...
import io.github.eastseao.markdownhelper.web.MarkdownHelperWebViewClient;
import io.github.eastseao.markdownhelper.widget.TodoWidgetProvider;
...
public class DocumentEditAndViewFragment extends MarkdownHelperBaseFragment implements FormatRegistry.TextFormatApplier {
    ...
    /** Log tag for share-as-image diagnostics (keep in sync with GsContextUtils). */
    private static final String TAG_SHARE_IMAGE = "MdShareImage";

    /**
     * JPEG quality used for the exported document image. MarkdownH's default is 70, which shows
     ...
     */
    private static final int SHARE_IMAGE_JPEG_QUALITY = 95;

    /**
     * Time given to the {@link android.webkit.WebView} to lay the document out again after the
     * view was resized for the export. The renderer lays out in a separate process, so capturing
     * in the very same frame would still return the previous layout.
     */
    private static final int EXPORT_LAYOUT_SETTLE_MS = 2000;
    ...
    private MarkdownHelperWebViewClient _webViewClient;
    ...
    private MarkdownHelperContextUtils _cu;
    ...
    public void onViewCreated(@NonNull View view, Bundle savedInstanceState) {
        ...
        _cu = new MarkdownHelperContextUtils(activity);
    ...
    /**
     * Width (px) the share-as-image export should be produced at.
     * <p>
     * Portrait: the width configured in the settings is used.
     * <p>
     * Landscape: the current (wide) screen width is used instead. The document is laid out for the
     * wide viewport in landscape, so exporting at the narrow configured width would squeeze the
     * wide layout into a portrait sized canvas. Using the screen width keeps the exported image
     * matching what is actually shown on screen.
     */
    private int getShareImageExportWidthPx() {
        if (getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE) {
            try {
                final DisplayMetrics dm = getResources().getDisplayMetrics();
                final int screenWidth = Math.max(dm.widthPixels, dm.heightPixels);
                if (screenWidth > 0) {
                    Log.i(TAG_SHARE_IMAGE, "landscape export width=" + screenWidth
                            + " (webViewWidth=" + (_webView != null ? _webView.getWidth() : -1) + ")");
                    return screenWidth;
                }
            } catch (Exception ignored) {
            }
        }
        return _appSettings.getShareImageExportWidth();
    }
        ...
        switch (itemId) {
            ...
            case R.id.action_share_pdf: {
                ...
                if (saveDocument(false)) {
                    ...
                    if (_webView != null) {
                        // In landscape the document is laid out for the wide viewport, so the export
                        // must follow the on-screen width; otherwise the wide layout ends up squeezed
                        // into the (portrait oriented) configured width. In portrait the configured
                        // width is used as before.
                        final int exportWidthPx = getShareImageExportWidthPx();

                        // Resolution multiplier (image export only - a screenshot/PDF is already
                        // printed at its natural size).
                        final int exportScale = itemId == R.id.action_share_image
                                ? _appSettings.getShareImageExportScale() : 1;
                        ...
                        _webView.postDelayed(() -> {
                            ...
                            final boolean fullpage = itemId == R.id.action_share_image;
                            final int captureWidthPx = Math.max(320, exportWidthPx * exportScale);
                            final WebSettings webSettings = _webView.getSettings();
                            final int previousTextZoom = webSettings.getTextZoom();
                            final int scaledTextZoom = Math.round(previousTextZoom * exportScale);

                            // The renderer derives its layout viewport (CSS pixels) from the WebView
                            // size divided by the screen density - measured, not guessed: setting only
                            // the font size (textZoom) scales the glyphs but keeps the viewport, so the
                            // text would simply overflow and be clipped. Therefore the view itself has
                            // to become captureWidthPx wide, and because a real view size change is
                            // required (not just a measure()/layout() call inside the capture) the
                            // LayoutParams are used here and the renderer gets time to re-flow.
                            final ViewGroup.LayoutParams lp = _webView.getLayoutParams();
                            final int previousLayoutWidth = lp != null ? lp.width : ViewGroup.LayoutParams.MATCH_PARENT;
                            if (lp != null) {
                                lp.width = captureWidthPx;
                                _webView.setLayoutParams(lp);
                            }
                            if (scaledTextZoom != previousTextZoom) {
                                webSettings.setTextZoom(scaledTextZoom);
                            }
                            Log.i(TAG_SHARE_IMAGE, "export prepare: scale=" + exportScale
                                    + " viewWidth=" + captureWidthPx
                                    + " textZoom " + previousTextZoom + " -> " + scaledTextZoom);

                            // Give the WebView/renderer time to lay the document out for the new
                            // viewport before it is captured.
                            _webView.postDelayed(() -> {
                                Bitmap bmp = null;
                                try {
                                    bmp = _cu.getBitmapFromWebView(_webView, captureWidthPx, fullpage);
                                } finally {
                                    // Put the preview back the way the user has configured it
                                    if (lp != null) {
                                        lp.width = previousLayoutWidth;
                                        _webView.setLayoutParams(lp);
                                    }
                                    if (scaledTextZoom != previousTextZoom) {
                                        webSettings.setTextZoom(previousTextZoom);
                                    }
                                }
                                if (bmp == null) {
                                    Toast.makeText(activity, R.string.error_could_not_create_image, Toast.LENGTH_SHORT).show();
                                } else {
                                    _cu.shareImage(getContext(), bmp, null, SHARE_IMAGE_JPEG_QUALITY);
                                }
                            }, EXPORT_LAYOUT_SETTLE_MS);
                        }, 7000);
                    }
                }
            ...
            case R.id.action_load_epub: {
                MarkdownHelperFileBrowserFactory.showFileDialog(new GsFileBrowserOptions.SelectionListenerAdapter() {
                                                            @Override
            ...
            case R.id.action_set_font_size: {
                ...
                MarkdownHelperDialogFactory.showFontSizeDialog(activity, current, (newSize) -> {
                    if (_isPreviewVisible) {
                        if (_webView != null) {
    ...
    public void errorClipText() {
        ...
        if (!TextUtils.isEmpty(text)) {
            ...
            new MarkdownHelperContextUtils(context).setClipboard(getContext(), text);
        }
    ...
    private void setupWebViewIfNeeded(final Activity activity) {
        if (_webView == null) {
            ...
            _webViewClient = new MarkdownHelperWebViewClient(_webView, activity);
