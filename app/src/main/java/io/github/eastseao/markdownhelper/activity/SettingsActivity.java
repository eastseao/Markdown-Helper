...
import io.github.eastseao.markdownhelper.util.BackupUtils;
...
import other.writeily.widget.WrMarkdownHelperWidgetProvider;

public class SettingsActivity extends MarkdownHelperBaseActivity {
    ...
    public static abstract class MarkdownHelperSettingsFragment extends GsPreferenceFragmentBase<AppSettings> {
        @Override
    ...
    public static class SettingsFragmentMaster extends MarkdownHelperSettingsFragment {
        ...
        public void doUpdatePreferences() {
            ...
            // Width used when sharing / exporting a document as image
            final int shareImageWidth = _appSettings.getShareImageExportWidth();
            updateSummary(R.string.pref_key__share_image_export_width, shareImageWidth + " px");
            final Preference shareImageWidthPref = findPreference(R.string.pref_key__share_image_export_width);
            if (shareImageWidthPref instanceof EditTextPreference) {
                ((EditTextPreference) shareImageWidthPref).setOnBindEditTextListener(editText -> {
                    editText.setInputType(InputType.TYPE_CLASS_NUMBER);
                    editText.setSelection(editText.getText().length());
                });
            }

            // Resolution multiplier used when sharing / exporting a document as image
            final int shareImageScale = _appSettings.getShareImageExportScale();
            updateSummary(R.string.pref_key__share_image_export_scale, shareImageScale + "x");
        ...
        protected void onPreferenceChanged(final SharedPreferences prefs, final String key) {
            ...
            if (eq(key, R.string.pref_key__language)) {
                ...
                new MarkdownHelperContextUtils(getActivity()).applySpecialLaunchersVisibility(getActivity(), extraLaunchersEnabled);
            ...
            } else if (eq(key, R.string.pref_key__share_image_export_width)) {
                // Keep the stored value inside the allowed range and refresh the summary
                final int shareImageWidth = _appSettings.getShareImageExportWidth();
                _appSettings.setShareImageExportWidth(shareImageWidth);
                updateSummary(R.string.pref_key__share_image_export_width, shareImageWidth + " px");
            } else if (eq(key, R.string.pref_key__share_image_export_scale)) {
                // Keep the stored value inside the allowed range and refresh the summary
                final int shareImageScale = _appSettings.getShareImageExportScale();
                _appSettings.setShareImageExportScale(shareImageScale);
                updateSummary(R.string.pref_key__share_image_export_scale, shareImageScale + "x");
                ...
                WrMarkdownHelperWidgetProvider.updateLauncherWidgets();
        ...
        public Boolean onPreferenceClicked(Preference preference, String key, int keyResId) {
            ...
            switch (keyResId) {
                case R.string.pref_key__snippet_directory_path: {
                    MarkdownHelperFileBrowserFactory.showFolderDialog(new GsFileBrowserOptions.SelectionListenerAdapter() {
                        @Override
                ...
                case R.string.pref_key__quicknote_filepath: {
                    ...
                    }, fragManager, getActivity(), MarkdownHelperFileBrowserFactory.IsMimeText);
                    return true;
                }
                ...
                case R.string.pref_key__basic_color_scheme_markdownhelper: {
                ...
                case R.string.pref_key__set_encryption_password: {
                    MarkdownHelperDialogFactory.showSetPasswordDialog(getActivity());
                    break;
                }
