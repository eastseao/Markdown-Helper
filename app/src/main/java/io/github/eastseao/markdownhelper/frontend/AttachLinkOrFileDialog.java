...
import io.github.eastseao.markdownhelper.format.wikitext.WikitextLinkResolver;
...
public class AttachLinkOrFileDialog {
    ...
    private static GsCallback.b2<Context, File> getFilterForAction(final InsertType action) {
        switch (action) {
            ...
            case IMAGE_DIALOG:
            case IMAGE_BROWSE: {
                return MarkdownHelperFileBrowserFactory.IsMimeImage;
            }
            case AUDIO_RECORDING:
            case AUDIO_DIALOG:
            case AUDIO_BROWSE: {
                return MarkdownHelperFileBrowserFactory.IsMimeAudio;
            }
            case LINK_DIALOG:
            case LINK_BROWSE:
            case LINK_SPECIAL:
    ...
    private static void fetchAndInsertItem(
        ...
        final MarkdownHelperContextUtils cu = new MarkdownHelperContextUtils(activity);
        ...
        switch (action) {
            ...
            case AUDIO_BROWSE: {
                if (activity instanceof AppCompatActivity && nameEdit != null && pathEdit != null) {
                    ...
                    MarkdownHelperFileBrowserFactory.showFileDialog(fsListener, f, activity, getFilterForAction(action));
                }
                break;
            }
            case LINK_SPECIAL: {
                MarkdownHelperDialogFactory.showSelectSpecialFileDialog(activity, setFields);
                break;
            }
            case LINK_SEARCH: {
                MarkdownHelperDialogFactory.showNotebookFilterDialog(activity, null, null, (file, l) -> {
                    setFields.callback(file);
                });
            }
            case LINK_DIALOG:
